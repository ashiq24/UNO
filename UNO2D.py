...
class UNO_8(nn.Module):
    def __init__(self,in_width, width,pad = 6, factor = 3/4):
        ...
        self.conv3 = SpectralConv2d(8*factor*self.width, 16*factor*self.width, 8, 8,4,4)
        
        self.conv4 = SpectralConv2d(16*factor*self.width, 8*factor*self.width, 16, 16,4,4)
        

        self.conv5 = SpectralConv2d(16*factor*self.width, 4*factor*self.width, 32, 32,8,8)

        self.conv6 = SpectralConv2d(8*factor*self.width, 2*factor*self.width, 48, 48,16,16)

        self.conv7 = SpectralConv2d(4*factor*self.width, self.width, 64, 64,24,24) 
        ...
        self.w4 = pointwise_op(16*factor*self.width, 8*factor*self.width, 16, 16)
        
        self.w5 = pointwise_op(16*factor*self.width, 4*factor*self.width, 32, 32) #
        
        self.w6 = pointwise_op(8*factor*self.width, 2*factor*self.width, 48, 48)
        
        self.w7 = pointwise_op(4*factor*self.width, self.width, 64, 64) 
    ...
    def forward(self, x):
        ...
        x1_c6 = self.conv6(x_c5,int(D1*self.factor),int(D2*self.factor))
        x2_c6 = self.w6(x_c5,int(D1*self.factor),int(D2*self.factor))
        x_c6 = x1_c6 + x2_c6
        x_c6 = F.gelu(x_c6)
        x_c6 = torch.cat([x_c6, x_c0], dim=1)

        x1_c7 = self.conv7(x_c6,D1,D2)
        x2_c7 = self.w7(x_c6,D1,D2)
        x_c7 = x1_c7 + x2_c7
        x_c7 = F.gelu(x_c7)
        

        x_c7 = torch.cat([x_c7, x_fc0], dim=1)
        if self.padding!=0:
            x_c7 = x_c7[..., :-self.padding, :-self.padding]

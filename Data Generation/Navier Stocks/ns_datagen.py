...
def navier_stokes_2d(w0, f, visc, T, delta_t=1e-4, record_steps=1):
    ...
    for j in range(steps):
        ...
        if (j + 1) % record_time == 0:
            ...
            w = torch.irfft(
                w_h, 2, normalized=False, onesided=False, signal_sizes=(N, N)
            )
...
for j in range(N // bsize):
    ...
    if j == 0:
        scipy.io.savemat(
            "NS_data/ns_data_2_1200_T50_v001_dt0001.mat",
            mdict={
                "a0": w0.cpu().numpy(),
                "u0": sol.cpu().numpy(),
                "t0": sol_t.cpu().numpy(),
        ...
        with open("NS_data/ns_data_2_1200_T50_v001_dt0001.mat", "ab") as file1:
            scipy.io.savemat(
                file1,
                mdict={
                    "a" + str(j): w0.cpu().numpy(),
                    "u" + str(j): sol.cpu().numpy(),
                    "t" + str(j): sol_t.cpu().numpy(),
                },
                do_compression=True,
            )
    print(j, c, t1 - t0)

...
class Uno3D_T40(nn.Module):
    ...
    def __init__(self, in_width, width, pad=2, factor=1, pad_both=False):
        ...
        self.conv0 = OperatorBlock_3D(
            self.width, 2 * factor * self.width, 48, 48, 10, 20, 20, 4, Normalize=True
        )

        self.conv1 = OperatorBlock_3D(
            2 * factor * self.width, 4 * factor * self.width, 32, 32, 10, 14, 14, 4
        )

        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 16, 16, 16, 6, 6, 4
        )

        self.conv3 = OperatorBlock_3D(
            8 * factor * self.width,
            16 * factor * self.width,
            16,
            16,
            16,
            6,
        ...
        self.conv6 = OperatorBlock_3D(
            16 * factor * self.width, 4 * factor * self.width, 32, 32, 24, 6, 6, 7
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 64, 64, 40, 20, 20, 14
        )  # will be reshaped
    ...
    def forward(self, x):
        ...
        self.padding = int(self.pad * 0.1 * x_fc0.shape[-1])
        if self.pad_both:
            x_fc0 = F.pad(
                x_fc0, [self.padding, self.padding, 0, 0, 0, 0], mode="constant"
            )
        else:
        ...
        x_c6 = torch.cat(
            [
                ...
                torch.nn.functional.interpolate(
                    x_c1,
                    size=(x_c6.shape[2], x_c6.shape[3], x_c6.shape[4]),
                    mode="trilinear",
                    align_corners=True,
                ),
            ],
        ...
        x_c7 = torch.cat(
            [
                ...
                torch.nn.functional.interpolate(
                    x_c0,
                    size=(x_c7.shape[2], x_c7.shape[3], x_c7.shape[4]),
                    mode="trilinear",
                    align_corners=True,
                ),
            ],
        ...
        x_c8 = torch.cat(
            [
                ...
                torch.nn.functional.interpolate(
                    x_fc0,
                    size=(x_c8.shape[2], x_c8.shape[3], x_c8.shape[4]),
                    mode="trilinear",
                    align_corners=True,
                ),
            ],
...
class Uno3D_T20(nn.Module):
    ...
    def __init__(self, in_width, width, pad=2, factor=1, pad_both=False):
        ...
        self.conv0 = OperatorBlock_3D(
            self.width, 2 * factor * self.width, 48, 48, 10, 22, 22, 5, Normalize=True
        )

        self.conv1 = OperatorBlock_3D(
            2 * factor * self.width, 4 * factor * self.width, 32, 32, 10, 14, 14, 5
        )

        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 16, 16, 12, 6, 6, 5
        )
        ...
        self.conv6 = OperatorBlock_3D(
            16 * factor * self.width, 4 * factor * self.width, 32, 32, 18, 6, 6, 6
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 64, 64, 20, 22, 22, 8
        )  # will be reshaped
...
class Uno3D_T10(nn.Module):
    ...
    def __init__(self, in_width, width, pad=2, factor=1, pad_both=False):
        ...
        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 16, 16, 10, 6, 6, 5
        )
        ...
        self.conv6 = OperatorBlock_3D(
            16 * factor * self.width, 4 * factor * self.width, 32, 32, 10, 6, 6, 5
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 64, 64, 10, 22, 22, 5
        )  # will be reshaped
...
class Uno3D_T9(nn.Module):
    ...
    def __init__(self, in_width, width, pad=2, factor=1, pad_both=False):
        ...
        self.conv0 = OperatorBlock_3D(
            self.width, 2 * factor * self.width, 48, 48, 6, 20, 20, 3, Normalize=True
        )

        self.conv1 = OperatorBlock_3D(
            2 * factor * self.width, 4 * factor * self.width, 32, 32, 6, 18, 18, 3
        )

        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 16, 16, 6, 6, 6, 3
        )
        ...
        self.conv6 = OperatorBlock_3D(
            16 * factor * self.width, 4 * factor * self.width, 32, 32, 8, 6, 6, 3
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 64, 64, 9, 20, 20, 4
        )  # will be reshaped
...
class Uno3D_T40_256(nn.Module):
    def __init__(self, in_width, width, pad=1, factor=1):
        ...
        self.conv0 = OperatorBlock_3D(
            self.width, 2 * factor * self.width, 64, 64, 10, 32, 32, 5, Normalize=True
        )

        self.conv1 = OperatorBlock_3D(
            2 * factor * self.width, 4 * factor * self.width, 16, 16, 10, 8, 8, 5
        )

        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 8, 8, 16, 4, 4, 5
        )
        ...
        self.conv4 = OperatorBlock_3D(
            16 * factor * self.width, 16 * factor * self.width, 8, 8, 16, 4, 4, 8
        )

        self.conv5 = OperatorBlock_3D(
            16 * factor * self.width,
            8 * factor * self.width,
            8,
            8,
        ...
        self.conv6 = OperatorBlock_3D(
            8 * factor * self.width, 4 * factor * self.width, 16, 16, 24, 4, 4, 8
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 256, 256, 40, 32, 32, 16
        )  # will be reshaped
...
class Uno3D_T20_256(nn.Module):
    def __init__(self, in_width, width, pad=2, factor=1, pad_both=False):
        ...
        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 8, 8, 12, 4, 4, 5
        )
        ...
        self.conv4 = OperatorBlock_3D(
            16 * factor * self.width, 16 * factor * self.width, 8, 8, 16, 4, 4, 6
        )
        ...
        self.conv6 = OperatorBlock_3D(
            8 * factor * self.width, 4 * factor * self.width, 16, 16, 18, 4, 4, 8
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 256, 256, 20, 32, 32, 8
        )  # will be reshaped
...
class Uno3D_T10_256(nn.Module):
    def __init__(self, in_width, width, pad=2, factor=1, pad_both=False):
        ...
        self.conv1 = OperatorBlock_3D(
            2 * factor * self.width, 4 * factor * self.width, 16, 16, 10, 8, 8, 4
        )

        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 8, 8, 8, 4, 4, 4
        )
        ...
        self.conv4 = OperatorBlock_3D(
            16 * factor * self.width, 16 * factor * self.width, 8, 8, 8, 4, 4, 4
        )
        ...
        self.conv6 = OperatorBlock_3D(
            8 * factor * self.width, 4 * factor * self.width, 16, 16, 8, 4, 4, 4
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 256, 256, 10, 32, 32, 5
        )  # will be reshaped
...
class Uno3D_T9_256(nn.Module):
    def __init__(self, in_width, width, pad=2, factor=1, pad_both=False):
        ...
        self.conv0 = OperatorBlock_3D(
            self.width, 2 * factor * self.width, 64, 64, 6, 32, 32, 3, Normalize=True
        )

        self.conv1 = OperatorBlock_3D(
            2 * factor * self.width, 4 * factor * self.width, 16, 16, 6, 8, 8, 3
        )

        self.conv2 = OperatorBlock_3D(
            4 * factor * self.width, 8 * factor * self.width, 8, 8, 6, 4, 4, 3
        )
        ...
        self.conv8 = OperatorBlock_3D(
            4 * factor * self.width, 2 * self.width, 256, 256, 9, 32, 32, 4
        )  # will be reshaped
